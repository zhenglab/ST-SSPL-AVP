...
from lib.core import Hook, metric, Recorder, get_priority, hook_maps
from lib.methods import method_maps
from lib.utils import (set_seed, print_log, output_namespace, check_dir, collect_env,
...
class BaseExperiment(object):
    ...
    def _load_from_state_dict(self, state_dict):
        if self._dist:
            ...
            try:
                ...
                self.method.model.load_state_dict(state_dict_new)

    def display_method_info(self):
        """Plot the basic infomation of supported methods"""
        T, C, H, W = self.args.in_shape
        if self.args.method in ['mgma']:
    ...
    def test(self):
        ...
        if 'weather' in self.args.dataname or 'ocean' in self.args.dataname:
    ...
    def vis(self, vis_path, data_name):
        """A testing loop of STL methods"""
        if self.args.test:
            best_model_path = osp.join(self.path, 'checkpoint.pth')
            self._load_from_state_dict(torch.load(best_model_path))
        ...
        self.call_hook('before_val_epoch')
        for i in range(1,7):
            self.method.vis_one_epoch(self, self.test_loader, vis_path, data_name, i)
        self.call_hook('after_val_epoch')
